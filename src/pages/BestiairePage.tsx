...
import { creatures, getUniverseName } from "@/data/mockData";
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {creatures.map((cr, i) => (
            <motion.div
              ...
              className="grimoire-card p-6 cursor-pointer group"
            >
              ...
              <p className="text-xs text-muted-foreground mt-1">🌍 {getUniverseName(cr.universeId)}</p>
              <p className="text-xs text-primary/80 mt-1">Danger: {dangerStars(cr.dangerLevel)}</p>
        ...
        {sel && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
            ...
            <motion.div
              ...
              className="relative grimoire-card p-8 max-w-lg w-full glow-gold"
            ...
            >
              ...
              <p className="text-sm text-primary/80 mt-1">Danger: {dangerStars(sel.dangerLevel)}</p>
              <p className="text-xs text-muted-foreground">🌍 {getUniverseName(sel.universeId)} · 📍 {sel.habitat}</p>
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {sel.abilities.map(a => (
                    <span key={a} className="text-xs bg-primary/10 text-primary px-2 py-1 rounded font-cinzel">{a}</span>
                  ))}
                </div>
              </div>
            </motion.div>
